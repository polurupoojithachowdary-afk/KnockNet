...
document.addEventListener('DOMContentLoaded', () => {
  ...
  btnCreateRoom.addEventListener('click', async () => {
    ...
    btnCreateRoom.innerHTML = '<span>Provisioning Mesh...</span>';
    ...
    try {
      const res = await fetch('/api/rooms', {
        ...
        body: JSON.stringify({
          ...
          hostName: hostSessionTitle.value.trim() || 'Host'
        })
      });
      ...
      const fullJoinUrl = window.location.origin + data.joinUrl;
      shareLinkInput.value = fullJoinUrl;
      
      btnEnterRoom.onclick = () => {
        sessionStorage.setItem('knocknet_displayName', hostSessionTitle.value.trim() || 'Host');
        window.location.href = data.joinUrl;
      };

      roomCreatedCard.classList.add('active');
      startCountdownTimer(data.codeSecondsRemaining || 60);
      showToast('Room provisioned. Active 6-digit code generated.');
      ...
      btnCreateRoom.innerHTML = `
        <span>Initiate Conference Room</span>
  ...
  const joinUserName = document.getElementById('join-user-name');
  ...
  btnSubmitCode.addEventListener('click', async () => {
    ...
    const displayName = joinUserName.value.trim() || 'Peer-' + Math.floor(100 + Math.random() * 900);
    ...
    try {
      ...
      sessionStorage.setItem('knocknet_displayName', displayName);
      showToast('Token verified. Entering conference...');
      setTimeout(() => {
        window.location.href = data.joinUrl;
      }, 400);
  ...
  function renderMeetingsList(meetings) {
    ...
    meetings.forEach(m => {
      ...
      item.querySelector('.btn-launch-schedule').addEventListener('click', () => {
        ...
        window.location.href = `/room.html?id=${m.roomId}`;
      });
