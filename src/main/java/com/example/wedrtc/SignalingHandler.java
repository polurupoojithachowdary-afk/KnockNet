...
public class SignalingHandler extends TextWebSocketHandler {
    ...
    private void handleJoin(WebSocketSession session, JsonNode json) throws IOException {
        ...
        String displayName = json.path("displayName").asText("Guest");

        if (roomId.isBlank() || userId.isBlank()) {
            sendJson(session, Map.of("type", "error", "message", "Missing roomId or userId"));
            return;
        }
    ...
    private void handleRelay(WebSocketSession session, JsonNode json, String type) throws IOException {
        ...
        if (targetUserId.isBlank() || fromUserId == null) {
