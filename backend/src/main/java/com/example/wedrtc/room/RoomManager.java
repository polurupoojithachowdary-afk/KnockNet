...
public class RoomManager {
    ...
    private final Map<String, ConcurrentLinkedDeque<Instant>> joinAttemptsByUser = new ConcurrentHashMap<>();

    public RoomManager() {
    }

    public ScheduledMeeting scheduleMeeting(String title, String scheduledTime, int maxParticipants, String hostName, String hostId) {
        Room room = createRoom(hostId, maxParticipants);
        ScheduledMeeting meeting = new ScheduledMeeting(title, scheduledTime, maxParticipants, hostName, hostId, room.getId(), room.getCode());
        scheduledMeetings.add(meeting);
        log.info("Scheduled conference created: title={}, time={}, roomId={}", title, scheduledTime, room.getId());
        return meeting;
    }

    public List<ScheduledMeeting> getScheduledMeetings(String hostId) {
        return scheduledMeetings.stream().filter(meeting -> meeting.belongsTo(hostId)).toList();
    }

    public boolean deleteScheduledMeeting(String id, String hostId) {
        return scheduledMeetings.removeIf(m -> m.getId().equals(id) && m.belongsTo(hostId));
    }
     ...
     */
    public RoomValidationResult validateAndAuthorizeByCode(String code, String userId) {
        if (!allowJoinAttempt(userId)) {
            return RoomValidationResult.rateLimited("Too many attempts. Wait one minute before trying again.");
        }
        ...
        room.authorizeParticipant(userId);
        joinAttemptsByUser.remove(userId);
        return RoomValidationResult.valid(room);
    }

    private boolean allowJoinAttempt(String userId) {
        if (userId == null || userId.isBlank()) return false;
        Instant cutoff = Instant.now().minusSeconds(60);
        ConcurrentLinkedDeque<Instant> attempts = joinAttemptsByUser.computeIfAbsent(
                userId, ignored -> new ConcurrentLinkedDeque<>());
        while (attempts.peekFirst() != null && attempts.peekFirst().isBefore(cutoff)) {
            attempts.pollFirst();
        }
    ...
    public static class RoomValidationResult {
        public enum Status { VALID, EXPIRED, FULL, INVALID, RATE_LIMITED }
        ...
        public static RoomValidationResult rateLimited(String message) {
            return new RoomValidationResult(Status.RATE_LIMITED, message, null);
        }
